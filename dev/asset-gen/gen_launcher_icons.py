...
ROOT = os.path.join(_PROJECT_ROOT, "resources", "icons")
TILES_ROOT = os.path.join(_PROJECT_ROOT, "resources", "tiles")
BG_ROOT = os.path.join(_PROJECT_ROOT, "resources", "background")
STORE_ROOT = os.path.join(_PROJECT_ROOT, "docs", "store-assets")
...
def render_splash_icon(
	godot_tiles: list[tuple[int, int, int, int]] | None,
	size: int,
) -> list[tuple[int, int, int, int]]:
	"""Opaque void square with four tiles only — no stars (Android grays transparent pixels)."""
	rgb = hex_rgb(BG_VOID)
	out = [(*rgb, 255)] * (size * size)
	icon = scale_nn(
...
def write_png_rgb(path: str, w: int, h: int, pixels: list[tuple[int, int, int, int]]) -> None:
	"""RGB PNG (no alpha) so Android never treats splash pixels as transparent."""
	raw = bytearray()
	i = 0
	for _y in range(h):
		raw.append(0)
...
def main() -> None:
	...
	# Opaque void fill so transparent corners never show OS gray on the splash.
	splash = render_splash_icon(godot_tiles, 256)
	write_png_rgb(os.path.join(ROOT, "splash_app_icon_256.png"), 256, 256, splash)
	splash_432 = render_splash_icon(godot_tiles, 432)
	write_png_rgb(os.path.join(ROOT, "splash_app_icon_432.png"), 432, 432, splash_432)
