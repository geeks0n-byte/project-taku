...
class AndroidExportPlugin extends EditorExportPlugin:
	...
	func _export_begin(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> void:
		...
		if id.is_empty():
			printerr(
				"[{plugin_name}] Export game id is empty; Play Games manifest omitted.".format(
					{"plugin_name": _plugin_name}
				)
			)
			print(
				"Set Project > Export > Android > godot_play_game_services/game_id when Play Games is needed."
			)
			return

		DirAccess.make_dir_recursive_absolute("res://android/build/res/values/")
		var file := FileAccess.open("res://android/build/res/values/strings.xml", FileAccess.WRITE)
		if file == null:
			printerr("[{plugin_name}] Failed to write android/build/res/values/strings.xml".format(
				{"plugin_name": _plugin_name}
			))
			return
		file.store_string(
		...
		print(
			"[{plugin_name}] added game-id {id} to [values/strings.xml]".format(
				{"plugin_name": _plugin_name, "id": id}
			)
		)
	...
	func _game_id() -> String:
		return str(get_option("godot_play_game_services/game_id")).strip_edges()
