...
func load(
	...
	if _plugin:
		...
		safe_connect(
			...
			_on_rewarded_interstitial_ad_loaded,
			CONNECT_DEFERRED
		)
		safe_connect(
			_plugin,
			"on_rewarded_interstitial_ad_failed_to_load",
			_on_rewarded_interstitial_ad_failed_to_load,
			CONNECT_DEFERRED
		)
		reference()
...
func _on_rewarded_interstitial_ad_failed_to_load(
	uid: int, load_ad_error_dictionary: Dictionary
) -> void:
	if uid == _uid:
		rewarded_ad_load_callback.on_ad_failed_to_load.call(
