...
func _init(uid: int) -> void:
	...
	if _plugin:
		...
		safe_connect(_plugin, "on_native_overlay_ad_video_start", _on_video_start)
		safe_connect(_plugin, "on_native_overlay_ad_video_play", _on_video_play)
...
static func load(
	...
	if _plugin:
		...
		safe_connect(
			...
			ad._on_ad_failed_to_load,
			CONNECT_DEFERRED
		)

		_plugin.load(
			ad_unit_id,
			ad_request.convert_to_dictionary(),
			ad_request.keywords,
			options.convert_to_dictionary(),
			uid
		)
...
func render_template(
	...
	if _plugin:
		...
		if ad_position.value == AdPosition.Values.CUSTOM:
			_plugin.render_template_custom_position(
				_uid,
				style.convert_to_dictionary(),
				ad_position.offset.x,
				ad_position.offset.y,
				ad_size_dict
			)
		else:
			_plugin.render_template(
				_uid, style.convert_to_dictionary(), ad_position.value, ad_size_dict
			)
...
func _on_ad_loaded(uid: int) -> void:
	if uid == _uid:
		...
		_media_content = MediaContent.new(_uid, _plugin)
