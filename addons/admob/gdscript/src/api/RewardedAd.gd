...
func show(on_user_earned_reward_listener := OnUserEarnedRewardListener.new()) -> void:
	if _plugin:
		...
		_plugin.show(_uid)
		safe_connect(
			_plugin, "on_rewarded_ad_user_earned_reward", _on_rewarded_ad_user_earned_reward
		)
...
func register_callbacks() -> void:
	if _plugin:
		safe_connect(_plugin, "on_rewarded_ad_clicked", _on_rewarded_ad_clicked)
		safe_connect(
			_plugin,
		...
		safe_connect(
			...
			_on_rewarded_ad_showed_full_screen_content
		)
...
func _on_rewarded_ad_failed_to_show_full_screen_content(
