...
def categorize(k: str) -> str:
	if k.startswith("TUT"):
		return "01_tutorial"
	if k.startswith("HTP_"):
		return "02_how_to_play"
	if k.startswith("STAR_") or k in {
		"LEVEL", "LOCKED", "DIFF_TUTORIALS", "DIFF_EASY", "DIFF_MEDIUM", "DIFF_HARD",
		"CUSTOM_LVL", "NO_PLAYABLE_LEVELS", "NO_CUSTOM_LEVELS", "UI_SELECT_LEVEL",
		"UI_LEVEL_SELECT", "UI_CORE", "UI_CUSTOM", "UI_LEVEL_STAR_GOALS",
		"UI_TAP_STARS_FOR_GOALS", "LEVEL_COMPLETED", "STAT_TIME_LEFT",
		"STAT_GREEN_TILES", "STAT_SHIFTER_MOVES", "TIMES_UP",
	}:
		return "03_levels_and_stars"
	if k.startswith("UI_") or k.startswith("CONFIRM_") or k.startswith("SESSION_") or k in {
		"SAVE_DELETED", "CUSTOM_DELETED", "UNLOCK_ALL_DONE",
	}:
		return "04_ui_menus"
	if k.startswith("MSG_") or k in {"MOVES", "COUNTER_GREEN", "DEV", "LVL", "TIME"}:
		return "05_gameplay_hud"
	if k.startswith("CREDITS") or k.startswith("SPLASH"):
		return "06_meta"
	if k in {
		"TUTORIAL", "COMPLETED", "YOU_WIN", "PLAY_AGAIN", "NEXT_LEVEL", "ALL_COMPLETED",
		"CUSTOM_COMPLETED", "PUZZLE_SOLVED", "LEVEL_SOLVABLE", "RETURN", "PAUSED",
		"COMPLETION_TIME", "TIMES_UP_UNSOLVED", "USED", "UNLIMITED",
	}:
		return "07_game_flow"
	if k.startswith("ERR_") or k.startswith("ERROR_") or k.startswith("WARN_"):
		return "09_errors"
	if k.startswith("ED_") or k in {
		"EDIT_MODE", "TEST_MODE", "LOCK_WALLS", "UNIQUE_SOLVE", "ALLOW", "Wall",
		"Empty (Clear)", "Yellow Tile", "Blue Tile", "Joker Tile",
		"Shifter Tile Link Tool", "Equals (=) Link Tool", "Not Equals (×) Link Tool",
		"LEVEL_EXISTS_OVERWRITE", "LEVEL IS SOLVABLE!",
	}:
		return "10_editor"
	return "08_other"
