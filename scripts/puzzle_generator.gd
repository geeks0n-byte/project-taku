...
static func generate_random_layout(width: int, height: int, allowed_tiles: Array) -> Dictionary:
	...
	# The solver is now allowed to use any tiles checked in the editor, 
	# including Jokers, which are strictly required to balance odd grids!
	var solver_tiles = allowed_tiles.duplicate()
	if solver_tiles.size() == 0: solver_tiles = [0, 1, 2]
	
	while attempt < 100: # Added a hard break to prevent freezing on mathematically impossible requests
		attempt += 1
		...
		var max_punch_time = 400 
		
		for c in filled_cells:
			if Time.get_ticks_msec() - hole_punch_start > max_punch_time: break
	...
	push_error("Generator failed: Board dimensions may be mathematically impossible with the allowed tiles.")
	return {} 
...
static func _is_valid_placement(coord: Vector2i, val: int, layout: Dictionary, w: int, h: int) -> bool:
	...
	# 3-in-a-row constraint (Jokers actively act as both colors for this check)
	...
	# Universal Balance Logic: Checks absolute difference against Joker count!
	var col_playable = 0; var col_filled = 0; var col_0 = 0; var col_1 = 0; var col_joker = 0
	for y in range(h):
		...
		if st != -2:
			...
			if st >= 0:
				...
				elif st == 2: col_joker += 1
				
	var max_col_allowed = int(col_playable / 2.0)
	if col_0 > max_col_allowed or col_1 > max_col_allowed:
		layout[coord] = -1
		return false
		
	if col_playable > 0 and col_playable == col_filled:
		if abs(col_0 - col_1) > col_joker:
			layout[coord] = -1
			return false
			
	var row_playable = 0; var row_filled = 0; var row_0 = 0; var row_1 = 0; var row_joker = 0
	for x in range(w):
		...
		if st != -2:
			...
			if st >= 0:
				...
				elif st == 2: row_joker += 1
				
	var max_row_allowed = int(row_playable / 2.0)
	if row_0 > max_row_allowed or row_1 > max_row_allowed:
		layout[coord] = -1
		return false
		
	if row_playable > 0 and row_playable == row_filled:
		if abs(row_0 - row_1) > row_joker:
			layout[coord] = -1
			return false
